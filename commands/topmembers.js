...
function topMembers(sock, chatId, isGroup) {
    if (!isGroup) {
        sock.sendMessage(chatId, { text: 'Cette commande est disponible uniquement dans un groupe.' });
        return;
    }
    ...
    if (sortedMembers.length === 0) {
        sock.sendMessage(chatId, { text: '📬 Aucune activité enregistrée pour le moment.' });
        return;
    }
