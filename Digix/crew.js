...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '50956649848@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *𝙎𝙐𝙆𝙐𝙉𝘼 𝙈𝘿 Bot Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. 𝙎𝙐𝙆𝙐𝙉𝘼 𝙈𝘿, one of the best."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 50956649848; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '50956649848' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'SUKUNAMD');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['50956649848@s.whatsapp.net'], // remplace par ton numéro WhatsApp 
